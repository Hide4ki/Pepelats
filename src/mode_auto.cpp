...
void ModeAuto::run()
{
    ...
    //pos_control.set_all_target(40.0);
	//_mode = Auto_TakeOff;
	...
	{
		motors->armed(true);
	    gcs().send_text(MAV_SEVERITY_CRITICAL, "Hello world 1");
		first = false;
	}
	else if(cnt ==50 && second)
	{
		AP_Mission::Mission_Command mycmd;
		mycmd.id = MAV_CMD_NAV_TAKEOFF;
		mycmd.p1 = 40.0;
	    gcs().send_text(MAV_SEVERITY_CRITICAL, "Hello world 2");
		start_command(mycmd);
		second = false;
	}
	else
...
void ModeAuto::takeoff_start(const Location& dest_loc)
{
    ...
    if (!copter.current_loc.initialised()) {
		...
		gcs().send_text(MAV_SEVERITY_CRITICAL, "in copter location check!!!");
    ...
    if (!wp_nav->set_wp_destination(dest)) {
		...
		gcs().send_text(MAV_SEVERITY_CRITICAL, "in waypoint controller");
	...
	gcs().send_text(MAV_SEVERITY_CRITICAL, "takeoff start fin");
...
{
	gcs().send_text(MAV_SEVERITY_CRITICAL, "in start_command");
    ...
    case MAV_CMD_NAV_TAKEOFF:                   // 22
		gcs().send_text(MAV_SEVERITY_CRITICAL, "in switch from start command");
        do_takeoff(cmd);
        break;
...
{
	gcs().send_text(MAV_SEVERITY_CRITICAL, "Sirprize");
    auto_takeoff_run();
...
{
	gcs().send_text(MAV_SEVERITY_CRITICAL, "in do takeoff");
